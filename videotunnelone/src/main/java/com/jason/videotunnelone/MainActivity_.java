 ...
 *
 */
public class MainActivity_ extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private void initData() {
        ...
        mLocalSurfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(@NonNull SurfaceHolder holder) {
                //1、 初始化 SurfaceView，这个方法非常重要，不初始化黑屏
                mLocalSurfaceView.init(eglBaseContext, null);
                mLocalSurfaceView.setScalingType(RendererCommon.ScalingType.SCALE_ASPECT_FILL);
                mLocalSurfaceView.setMirror(true); //是否镜像绘制
                mLocalSurfaceView.setEnableHardwareScaler(false);

                //2、 创建VideoCapturer(相机的描述符)，取得VideoCapturer中的VideoSource 用于关联视频轨道
                ...
                //4、将视轨与surfaceview结合
                mLocalVideoTrack.addSink(mLocalSurfaceView); // 添加渲染接收端器到轨道中，画面开始呈现
            }
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.peer_init) {
            initVideoConnection();
            localPeerConnection.addStream(localMediaStream);
        } else if (view.getId() == R.id.offer_video) {
            initVideoOffer();
        } else if (view.getId() == R.id.peer_init_b) {
            initVideoConnectionB();
        } else if (view.getId() == R.id.answer_video) {
            remotePeerConnection.setRemoteDescription(new SdpObserver("把 sdp 给到 remote"), localOfferSdp);
            initVideoAnswer();
        }
    }
    ...
    SessionDescription localOfferSdp;
    private void initVideoOffer() {
        //客户端创建 offer
...
//                remotePeerConnection.setRemoteDescription(new SdpObserver("把 sdp 给到 remote"), sessionDescription);
                localOfferSdp = sessionDescription;
...
//                }, new MediaConstraints());
    ...
    private void initVideoConnectionB() {
        List<PeerConnection.IceServer> iceServers = new ArrayList<>();
            ...
            public void onAddStream(MediaStream stream) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        stream.videoTracks.get(0).addSink(mRemoteSurfaceView);
                    }
                });
            }
            public void onIceCandidate(IceCandidate iceCandidate) { //3次
